...
class mapManipulator(Node):
    ...
    def make_likelihood_field(self):
        """
        Creates a likelihood field for the map based on occupied cells and laser sensor noise.
        This method computes the probability of a laser measurement at each cell in the map,
        assuming Gaussian noise with standard deviation laser_sig.

        Explanation for TA:
        - Identifies all occupied cells (pixels < 10) in the image array.
        - Converts occupied cell indices to world positions.
        - Generates positions for all cells in the map.
        - Uses a KDTree to find the distance from each cell to the nearest occupied cell.
        - Computes likelihood probabilities using a Gaussian: exp(-(dist^2)/(2*sigma^2)).
        - Reshapes the probabilities into the map shape to create the likelihood field.
        - Also creates a visual image of the likelihood field (inverted for display).
        - Stores the likelihood field, occupied points, and image for later use.

        The likelihood field represents how likely it is for a laser ray to hit an obstacle at each position,
        which is used in particle filter weight calculation to evaluate particle poses.
        """
